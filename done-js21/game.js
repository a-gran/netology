...
class Vector {
  ...
  // метод plus() создает и возвращает новый объект типа Vector, координаты которого будут суммой 
  //соответствующих координат суммируемых векторов
  plus(vector) {
    if(vector instanceof Vector === false) {
      throw new Error('Можно прибалять только тип Vector');
    }
    return new Vector(vector.x + this.x, vector.y + this.y);
  }

  // метод times() создает и возвращает новый объект типа Vector, координаты которого будут равны 
  ...
  times(numeric) {
    ...
    return new Vector(numeric * this.x, numeric * this.y);
  }
}
...
class Actor {
  ...
  constructor(pos = new Vector(), size = new Vector(1, 1), speed = new Vector()) {
    if((size instanceof Vector === false) || (pos instanceof Vector === false) || 
       (speed instanceof Vector === false)) {
...
//в которых установлены границы объекта по осям X и Y с учетом его расположения и размера
  ...
  isIntersect(moveActor) {
    ...
    return !(this === moveActor || 
             this.left >= moveActor.right || this.top >= moveActor.bottom || 
...
class Level {
  ...
  noMoreActors(typeMoveActor) {
    return this.actors.every(actor => actor.type !== typeMoveActor);
  }
...
// словарь движущихся объектов в игре
const actorDict = {
  '@': Player,
...
// запуск игры
// подключение JSON-файла с перечнем уровней
